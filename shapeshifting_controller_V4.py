...
Mutation responses:
    Port hopping      — drops traffic to the most-scanned port (port scan)
    Flow rule rewrite — installs timed DROP rule for offending source IP
                        (SYN flood, reconnaissance, brute force)

Run with:
    ryu-manager shapeshifting_controller_V3.py
...
class DetectionEngine:
    ...
    #  SYN flood: many SYNs, very few or zero ACKs
    # ------------------------------------------------------------------
    ...
    #  Reconnaissance generates many new flows very quickly.
    # ------------------------------------------------------------------
    ...
    #  Credential attacks generate a very high count on one (IP, port).
    # ------------------------------------------------------------------
...
#    Installs a timed DROP rule for the most-scanned port.
#    The rule auto-expires after HOP_RULE_TIMEOUT seconds.
#
#  Mutation ii — Flow rule rewrite (source block):
#    Installs a timed DROP rule for the top offending source IP.
#    The rule auto-expires after BLOCK_RULE_TIMEOUT seconds.
# ======================================================================
...
class MutationEngine:
    ...
    def respond(self, detection_result, datapaths,
        ...
        Receives traffic counters BEFORE they are reset so targets can be
        identified from the actual attack data.
        """
    ...
    #  MUTATION i — PORT HOPPING
    #
    #  Identifies the most-targeted destination port from the current
    #  detection window and installs a high-priority DROP rule for it.
    #  hard_timeout causes the rule to auto-expire after HOP_RULE_TIMEOUT
    #  seconds, after which normal forwarding resumes automatically.
    # ------------------------------------------------------------------
    def _port_hop(self, datapath, dst_port_counts):
        ...
        self.logger.warning(
            ...
            "(auto-expires)", target_port, HOP_RULE_TIMEOUT)
        self.logger.warning(
    ...
    #  MUTATION ii — FLOW RULE REWRITE (SOURCE BLOCK)
    #
    #  Identifies the most active source IP from the current detection
    #  window and installs a high-priority DROP rule for all traffic
    #  from that IP. Rule auto-expires after BLOCK_RULE_TIMEOUT seconds.
    # ------------------------------------------------------------------
    def _block_source(self, datapath, src_ip_counts,
        ...
        self.logger.warning(
            ...
            "(auto-expires)", offender_ip, BLOCK_RULE_TIMEOUT)
...
# ======================================================================
#  RYU CONTROLLER
#  Handles all SDN communication, feeds DetectionEngine, triggers
...
class ShapeShiftingController(app_manager.RyuApp):
    ...
    # ------------------------------------------------------------------
    #  PACKET-IN — MAC learning, switching, feature extraction
    # ------------------------------------------------------------------
    ...
    def packet_in_handler(self, ev):
        ...
        # Feature extraction — IP and above only
    ...
    def _detection_loop(self):
        ...
        Order of operations is critical:
            1. Run detection engine (reads current counters)
            2. Pass counters to mutation engine if MALICIOUS
               (mutation engine needs the data to identify targets)
            3. Reset all counters
               (ensures next cycle reflects only fresh traffic)

        Resetting before calling the mutation engine was the bug in
        earlier versions — it caused the mutation engine to always
        receive empty counters and fail to identify any target.
        """
        while True:
            hub.sleep(DETECTION_INTERVAL)

            # Step 1 — run detection against current counters
            ...
            # Step 2 — trigger mutation engine BEFORE resetting counters
            ...
            # Step 3 — reset counters after mutation engine has read them
            self.dst_port_counts    = collections.Counter()
            self.src_ip_counts      = collections.Counter()
            self.tcp_flag_counts    = collections.Counter()
            self.connection_tracker = collections.Counter()
