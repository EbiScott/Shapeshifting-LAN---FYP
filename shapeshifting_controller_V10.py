...
MARKER_FILE  = os.path.join(SCRIPT_DIR, 'pending_marker.txt')
...
class ThreadedHTTPServer(ThreadingMixIn, HTTPServer):
    """Handles each request in a separate thread so curl never blocks."""
    daemon_threads = True


def run_api_server():
    server = ThreadedHTTPServer(('0.0.0.0', API_PORT), DashboardHandler)
...
class ShapeShiftingController(app_manager.RyuApp):
    ...
    def _detection_loop(self):
        while True:
            ...
            # Read marker file written by test_suite.py
            try:
                if os.path.exists(MARKER_FILE):
                    ...
                    if marker:
                        ...
                        self.logger.info("[MARKER] Attached to cycle: %s", marker)
